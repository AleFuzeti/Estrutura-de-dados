...
void setFullRect(void* Sdata, int Sid, char ScorC[], char ScorP[], double Sx, double Sy, double Sw, double Sh){
    setRectId(Sdata, Sid);
    setRectCorC(Sdata, ScorC);
    setRectCorP(Sdata, ScorP);
    setRectX(Sdata, Sx);
    setRectY(Sdata, Sy);
    setRectW(Sdata, Sw);
    setRectH(Sdata, Sh);
    setRectType(Sdata, 'r');
    setRectProtec(Sdata, 60);
    setRectVida(Sdata, 3);
}

void* CriaRetangulo(int id, char corC[], char corP[], double x, double y, double w, double h){
    PrivateRect* Rect = callocDataR();
    setFullRect(Rect,id,corC,corP,x,y,w,h);
    Rect->area = w*h;
    return Rect;
};
