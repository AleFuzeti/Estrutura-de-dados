...
void checkBE(void* be)
{
    struct BE* BE = be;
    void* list = BE->list, *aux, *anc = BE->anc, *pont = BE->pont;
    double bex = BE->bex;
    double bey = BE->bey;
    double ber = BE->ber;
...
void be(void* anc, void* tree, FILE* qry, FILE* qrysvg, FILE* qrytxt, double agress, void* pont)
{
    ...
    if(findInRegionCPT(tree, bex, bey, ber, list))
    {   
...
//se e sec
void sel(void* form, void* list, FILE* qrytxt, char* aux)
{
    void *form2;
...
void se(void* nau, void* tree, FILE* qry, FILE* qrysvg, FILE* qrytxt)
{
    int id;
    ...
    if(pArray[1])
    {
        sel(pArray[1], nau, qrytxt, "se");
    }
}

void sec(int nausCap[], void* nau, void* tree, FILE* qry, FILE* qrysvg, FILE* qrytxt)
{
    int id,j;
...
{
    struct TR* TR = extra;
    double trx = TR->trx, try = TR->try, dx = TR->dx, dy = TR->dy;
    int id = TR->id;
    ...
    switch(tipo){ // verifica se acertou e cria os clones
        ...
        if  (trx >= x && trx <= x + w && try >= y && try <= y + h){
            ...
            insertCPT(TR->tree, x, y, newform, NULL, NULL, NULL);
