...
void setFullCirc(void* Sdata, int Sid, char ScorC[], char ScorP[], double Sx, double Sy, double Sr){
    setCircId(Sdata, Sid);
    setCircCorC(Sdata, ScorC);
    setCircCorP(Sdata, ScorP);
    setCircX(Sdata, Sx);
    setCircY(Sdata, Sy);
    setCircR(Sdata, Sr);
    setCircType(Sdata, 'c');
    setCircProtec(Sdata, 60);
    setCircVida(Sdata, 2);
}

void* CriaCirculo(int id, char corC[], char corP[], double x, double y, double r){
    PrivateCirc* Circ = callocDataC();
    setFullCirc(Circ,id,corC,corP,x,y,r);
    Circ->area = 3.14*r*r;
