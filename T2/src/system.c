    ...
    int  opt;
        while ((opt = getopt(argc, argv, "e:f:o:q:")) != -1) { //realiza a leitura dos parametros
            ...
            case 'f':; //leitura do nome do arquivo .geo
                ...
                char* geofull = calloc(strlen(geoarq) + strlen(getBed(paths)) + 7, sizeof(char));
            ...
            case 'q':; //leitura do nome do arquivo .qry
                ...
                char* qryfull = calloc(strlen(qryarq) + strlen(getBed(paths)) + 6, sizeof(char)); 
...
/*readPrintGeo*/
void sp(double x, double y, Info i, double xc, double yc, double r, void *extra)
{
...
void savePrintCirc(CPTree tree, FILE* geo, FILE* svg)
{
    ...
    insertCPT(tree, x, y, newform, sp, sp, extra);
...
{ 
    ...
    { 
        ...
        { 
            se(ancoras, naus, tree, qry, qrysvg, qrytxt);
        }
        else if (!strcmp(func, "sec")) 
        { 
            sec(nausCap, ancoras, naus, tree, qry, qrysvg, qrytxt);
        }
...
bool checkDentro(void* n, double x, double y, double r){
    void* data = n;
